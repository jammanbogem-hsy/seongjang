...
- `/events/:eventId/submission` — 패들렛형 개인 전시 게시물 작성·자동저장·제출
...
주최자는 Canva처럼 하단 페이지뷰에서 슬라이드를 추가·복제·삭제하고 드래그 앤 드롭으로 순서를 바꿀 수 있으며, 단계명·제목·질문·도움말을 실시간 편집할 수 있습니다. Google Slides형 샌드박스에서는 슬라이드마다 텍스트·숫자 입력 블록을 최대 6개까지 배치하고 이동·크기 조절·필수 여부 설정을 할 수 있습니다. 입력 구조는 응답이 시작된 뒤에도 자동 저장되어 현재 참여자 화면에 실시간 반영되며, 참여자 입력폼은 주최자의 자유 배치 좌표와 분리한 반응형 그리드로 재구성해 화면 크기와 무관하게 입력란이 겹치지 않습니다. 기존 응답은 수정된 구조와 분리해 제출 당시 이름과 값으로 계속 표시합니다. 참여자의 구조화된 응답은 기존 내보내기와 호환되는 개인 답변으로 저장되고, 주최자는 응답 푸시 사이드바와 상세 모달에서 즉시 확인합니다. 응답 공개는 모인 개수가 표시된 큰 CTA에서 미리보기와 댓글 동시 공개 여부를 확인한 뒤 실행합니다. 단계별 타이머(1–180분)와 답변·댓글 공개 상태를 바꾸면 Cloud Functions가 권한을 검증하고 Firestore 트랜잭션으로 갱신하며, 연결된 참여자 화면은 실시간 listener로 같은 상태를 받습니다. 슬라이드 텍스트와 입력 블록은 0.9초 디바운스로 자동 저장되고 타이머 시간은 슬라이드 기본값으로 유지됩니다. 답변이나 초안이 있는 슬라이드는 삭제할 수 없고, 타이머가 진행 중인 현재 슬라이드도 기록 보호를 위해 삭제가 제한됩니다.
...
세션 진행 중 참여자는 현재 슬라이드에 한 가지 반응을 선택·변경·취소하고 전체 채팅을 보낼 수 있습니다. 주최자는 본문을 차지하지 않는 우측 `청중 패널`을 열어 단계별 반응과 대화를 실시간으로 확인하고, 새 메시지를 보내거나 특정 메시지에 답장하고 부적절한 메시지를 삭제할 수 있습니다. 참여자도 주최자·다른 참여자의 메시지에 답장할 수 있으며 작성자 역할과 인용 원문이 함께 저장되어 재접속 후에도 대화 맥락이 유지됩니다. 답변 공개는 보기·댓글 권한만 열며 입력을 마감하지 않아, 참여자는 타이머가 끝나거나 다음 단계로 이동할 때까지 공개된 생각을 보면서 자신의 답을 계속 제출·보완할 수 있습니다. 댓글은 참여자 답변 카드와 주최자 응답 상세 모달에 실시간 표시됩니다. 채팅은 280자, 사용자별 단계당 30개, 전송 간격 1.5초로 서버에서 제한되고 클라이언트의 Firestore 직접 쓰기는 차단됩니다.
참여자는 `내 작품`에서 고양이 표지 또는 공개 HTTPS 대표 이미지, 작품명, 한 줄 소개, 상세 이야기, 실행·GitHub 링크, 태그와 제작 회고를 하나의 게시물로 작성합니다. 작성 중에는 실제 전시 카드가 즉시 미리 보이고 준비 진행률과 자동저장 상태가 표시됩니다. 제출하면 주최자의 작품 목록에 도착하며, 주최자가 전시를 공개한 뒤에는 공개 카드와 상세 모달에서 작품 핵심·회고·링크를 볼 수 있습니다. 이미지 파일을 Firebase Storage에 중복 업로드하지 않고 기본 표지나 공개 HTTPS 주소를 저장해 기본 운영 비용을 낮췄습니다.
...
- 패들렛형 전시 게시물 작성기: 표지 선택, 카드 실시간 미리보기, 필수 항목 진행률, 초안 자동저장과 주최자 공개 승인
...
- 답변·작품·댓글·검토 작성란·슬라이드 편집·정리 세션 자동저장, 우측 청중 패널의 단계별 반응·역할 표시·인용 답장형 실시간 채팅
