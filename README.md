...
세션 진행 중 참여자는 현재 슬라이드에 한 가지 반응을 선택·변경·취소하고 라이브 채팅을 보낼 수 있습니다. 주최자는 같은 화면에서 단계별 반응 집계와 최신 채팅을 실시간으로 확인하며 부적절한 메시지를 삭제할 수 있습니다. 답변 공개는 보기·댓글 권한만 열며 입력을 마감하지 않아, 참여자는 타이머가 끝나거나 다음 단계로 이동할 때까지 공개된 생각을 보면서 자신의 답을 계속 제출·보완할 수 있습니다. 댓글은 참여자 답변 카드와 주최자 응답 상세 모달에 실시간 표시됩니다. 채팅은 280자, 참여자당 단계별 30개, 전송 간격 1.5초로 서버에서 제한되고 클라이언트의 Firestore 직접 쓰기는 차단됩니다.
