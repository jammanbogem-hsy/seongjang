...
주최자는 슬라이드 내용을 실시간 편집하고 단계별 타이머 시간을 조절하며, 참여자는 닉네임과 4자리 개인 입장코드로 입장해 단계별 개인 답변과 최종 작품을 제출합니다. 공개 시점 이후에는 서로의 답변에 댓글을 남길 수 있으며, 주최자는 모든 입력을 정리 세션에서 테마·하이라이트·요약으로 수합합니다. 발행된 결과는 읽기 전용 대시보드, iframe, JSON, CSV, Markdown과 행사 README로 재사용할 수 있습니다.
...
주최자가 큰 `세션 시작` CTA를 누르면 첫 슬라이드·타이머·행사 상태가 한 트랜잭션으로 열리고 신규 닉네임 등록은 마감됩니다. 기존 참여자의 닉네임·4자리 개인 입장코드 재입장은 계속 허용됩니다.
